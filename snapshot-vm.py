...
Rewrote with parameterization by pryorda
'''
...
def main():
    ...
    # Add -s option
    parser.add_option("-v", "--vcenter", dest='vcenter',
                  help="-v vcenter-ip or dns name")
    parser.add_option("-p", "--password", dest='password',
                  help="-p vcenter-password")
    parser.add_option("-u", "--user", dest='user',
                  help="-u vcenter-user")
    parser.add_option("-m", "--vm", dest='vm',
                  help="-m virtual-machine")
    parser.add_option("-o", "--operation", dest='operation',
                  help="-o create, remove, or revert")
    parser.add_option("-n", "--name", dest='snapname',
                  help="-n snapshot-name")
    ...
    if not options.vm or not options.vcenter or not options.password or not options.user or not options.vm or not options.operation or not options.snapname or not options.description:
        parser.error("Refer to usage for help")
        sys.exit()


    inputs = {'vcenter_ip': options.vcenter,
              'vcenter_password': options.password,
              'vcenter_user': options.user,
              'vm_name' : options.vm,
              #create, remove or revert
              'operation' : options.operation,
              'snapshot_name' : options.snapname,
    ...
    try:
        ...
        if operation == 'create':
            ...
            print "Creating snapshot %s for vm: %s" % (snapshot_name, vm_name)
